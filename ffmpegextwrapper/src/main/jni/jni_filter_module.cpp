...
Java_com_panda_org_ffmpegextwrapper_FFmpegWrapper_filter(
        JNIEnv *env,jobject thiz, jobject surface, jstring srcpath){
    LOGI("play");
        ...
        if (packet.stream_index == videoStream) {
            ...
            if (frameFinished) {
                ...
                if (av_buffersrc_add_frame(buffersrc_ctx, pFrame) < 0) {
                    LOGI("Could not av_buffersrc_add_frame");
                    break;
                }

                ret = av_buffersink_get_frame(buffersink_ctx, pFrame);
                if (ret < 0) {
                    LOGI("Could not av_buffersink_get_frame");
                    break;
                }
...
static JNINativeMethod FilterMethods[] =
{
    {"filter", "(Ljava/lang/Object;Ljava/lang/String;)I",(void *)Java_com_panda_org_ffmpegextwrapper_FFmpegWrapper_filter}
};

int register_android_jni_filter_module(JNIEnv* env, jclass clazz){
        return jniRegisterNativeMethods(env, clazz, FilterMethods, sizeof(FilterMethods) / sizeof(FilterMethods[0]));
}