...
import com.pumpkin.org.angrypandaav.mediac.vedio.p3.AvcDecoderP3;
...
public class AvcDecoderP3Thread extends Thread {

    private final static String TAG="AvcDecoderP3Thread";

    private final static String FILE_PATH= Environment.getExternalStorageDirectory().getAbsolutePath() + "/test1.h264";
    //自定义的log打印，可以无视
    //解码器
    private AvcDecoderP3 avcDecoderP3;
    //文件读取完成标识
     ...
     * 初始化解码器
     */
    public AvcDecoderP3Thread(SurfaceHolder holder) {
        this.holder=holder;
        avcDecoderP3=new AvcDecoderP3(holder);
        avcDecoderP3.startCodec();
    }
    ...
    private void onFrame(byte[] frame, int offset, int length) {
        if (avcDecoderP3 != null) {
            try {
                avcDecoderP3.onFrame(frame, offset, length);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
